...
const InternalSeatingPage: React.FC = () => {
    ...
    useEffect(() => {
        (async () => {
            try { setSeriesList(await SeatingService.getSeries('Internal').then(r => Array.isArray(r) ? r : [])); } catch { }
            try { setHalls(await SeatingService.getHalls(selectedSeries ? Number(selectedSeries) : undefined).then(r => Array.isArray(r) ? r : [])); } catch { toast.error('Failed to load halls'); }
    ...
    const loadSummary = useCallback(async () => {
        ...
        try {
            const data = await SeatingService.getAllocationSummary(selectedDate, selectedSession, selectedSeries ? Number(selectedSeries) : undefined);
            startTransition(() => setHallSummary(Array.isArray(data) ? data : []));
        } catch { toast.error('Failed to load summary'); }
        finally { setLoadingSummary(false); }
    }, [selectedDate, selectedSession, selectedSeries, startTransition]);
    ...
    useEffect(() => {
        const handleSync = async (e?: Event | MessageEvent) => {
            ...
            if (detailHall && (!targetRoomId || detailHall.hallId === targetRoomId)) {
                try {
                    const layout = await SeatingService.getHallLayout(detailHall.hallId, selectedSeries ? Number(selectedSeries) : undefined);
                    startTransition(() => {
    ...
    const openHallDetail = async (hs: HallSummary) => {
        ...
        setLastSubjects([]);
        try {
            const layout = await SeatingService.getHallLayout(hs.hallId, selectedSeries ? Number(selectedSeries) : undefined);
            startTransition(() => {
                setDetailBenches(layout.benches || []);
                setDetailTotalSeats(layout.totalSeats || 0);
            });
            if (selectedDate) {
                try {
                    const alloc = await SeatingService.getAllocationForHall(selectedDate, selectedSession, hs.hallId, selectedSeries ? Number(selectedSeries) : undefined);
    ...
    const handleClearHall = async () => {
        ...
        try { await SeatingService.clearAllocation(selectedDate, selectedSession, detailHall.hallId, selectedSeries ? Number(selectedSeries) : undefined); setDetailAssignments({}); toast.success('Hall cleared'); loadSummary(); } catch { toast.error('Failed to clear'); }
    };
    ...
    const handleClearAllAllocations = async () => {
        ...
        try {
            const r = await SeatingService.clearAllAllocations(selectedDate, selectedSession, selectedSeries ? Number(selectedSeries) : undefined);
    ...
    const handleSaveHall = async () => {
        if (!detailHall || !selectedDate) return;
        try {
            await SeatingService.saveAllocation({ 
                examDate: selectedDate, 
                session: selectedSession, 
                hallId: detailHall.hallId, 
                seriesId: selectedSeries ? Number(selectedSeries) : undefined,
                assignments: Object.values(detailAssignments).map(a => ({ seatId: a.seatId, studentId: a.studentId })) 
            });
            toast.success('Saved');
    ...
    const handleCardClearHall = async (h: HallSummary) => {
        ...
        try { await SeatingService.clearAllocation(selectedDate, selectedSession, h.hallId, selectedSeries ? Number(selectedSeries) : undefined); toast.success(`${h.hallCode} cleared`); loadSummary(); } catch { toast.error('Failed to clear'); }
    };
    ...
    const buildGlobalRows = async () => {
        ...
        try {
            const { allocations } = await SeatingService.getGlobalAllocations(selectedDate, selectedSession, selectedSeries ? Number(selectedSeries) : undefined);
                            ...
                            <CardBody className="px-5 py-5 flex flex-col gap-5">
                                ...
                                <div className="space-y-3">
                                    ...
                                    <Select aria-label="Exam Series" placeholder="Select Exam Series" variant="bordered"
                                        ...
                                        {seriesList.filter(s => s.ExamType === 'Internal').map(s => <SelectItem key={String(s.ExamSeriesID)} textValue={s.SeriesName} className="data-[hover=true]:bg-indigo-50 data-[hover=true]:text-indigo-700 font-semibold">{s.SeriesName}</SelectItem>)}
                                    </Select>
                                ...
                                <div className={`space-y-3 transition-opacity ${!selectedDate ? 'opacity-40 pointer-events-none' : ''}`}>
                                    ...
                                    <div className="flex flex-col gap-1 px-4 py-3 bg-white border border-slate-200 rounded-xl shadow-sm">
                                        <span className="text-[12px] font-bold text-slate-800">
                                            {lastExamType === 'Internal' ? 'Subject-Based Auto Alignment' : 'Auto Balanced Allocation'}
                                        </span>
                                    ...
                                    {assignmentFeedback && (
                                        <div className="rounded-xl border border-slate-200 bg-slate-50 p-3 space-y-2">
                                            <div className="flex items-center justify-between">
                                                ...
                                                <span className={`text-[9px] font-extrabold uppercase tracking-widest px-2 py-0.5 rounded-full border ${lastExamType === 'Internal'
                                                    ? 'bg-violet-50 text-violet-700 border-violet-200'
                                                    : 'bg-indigo-50 text-indigo-700 border-indigo-200'
                                                    }`}>{lastExamType === 'Internal' ? 'End Semester' : 'Internal'}</span>
                                            </div>
                                            ...
                                            {lastExamType === 'Internal' && lastSubjects.length > 0 && (
                                ...
                                <div>
                                    ...
                                    {detailFilled > 0 && (
                                        <div className="flex flex-wrap items-center gap-2 mb-8">
                                            {/* ExamType badge */}
                                            <span className={`text-[9px] font-extrabold uppercase tracking-widest px-2.5 py-1 rounded-full border mr-1 ${lastExamType === 'Internal'
                                                ? 'bg-violet-900/40 text-violet-300 border-violet-500/40'
                                                : 'bg-indigo-900/40 text-indigo-300 border-indigo-500/40'
                                                }`}>
                                                {lastExamType === 'Internal' ? 'End Semester' : 'Internal'}
                                            </span>
                                    ...
                                    <div className="flex gap-6 justify-center items-start flex-wrap pb-8">
                                        {detailBenchRows.map((row, rowIdx) => {
                                            const isEndSem = lastExamType === 'Internal';
                                            return (
