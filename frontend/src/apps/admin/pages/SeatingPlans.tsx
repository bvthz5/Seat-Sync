...
/* ΓöÇΓöÇΓöÇ High-End Dark NASA Theme Colors ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
...
const SeatingPlans: React.FC = () => {
    ...
    /* ΓöÇΓöÇ Build row data shared by both exporters ΓöÇΓöÇ */
    ...
    const downloadExcel = async () => {
        ...
        const wsData = [
            [`Seating Arrangement ΓÇö ${detailHall?.hallCode}`],
            ...
            [],
            ['Bench', 'Left ΓÇö Reg No', 'Left ΓÇö Student Name', 'Left Dept', 'Right ΓÇö Reg No', 'Right ΓÇö Student Name', 'Right Dept'],
    ...
    const downloadPDF = async () => {
        ...
        autoTable(doc, {
            ...
            didDrawPage: (data: any) => {
                ...
                doc.text(
                    `Page ${data.pageNumber} of ${pCount}  ┬╖  CONFIDENTIAL ΓÇö FOR OFFICIAL USE ONLY`,
                    pageW / 2, doc.internal.pageSize.getHeight() - 6, { align: 'center' }
                );
            },
        });

        // ΓöÇΓöÇ Signature strip on last page ΓöÇΓöÇ
    ...
    /* ΓöÇΓöÇ Helper: build compact register-number ranges ΓöÇΓöÇ
       e.g. ["23CS001","23CS002","23CS003","23CS005"] ΓåÆ ["23CS001-003", "23CS005"] */
    ...
    /* ΓöÇΓöÇ Fetch all hall allocations and return consolidated rows ΓöÇΓöÇ */
    const buildGlobalRows = async () => {
        ...
        active.forEach(hall => {
            ...
            if (!depts.length) {
                rows.push({ slNo: slNo++, hallCode: hall.hallCode, regRanges: 'ΓÇö', count: 0, total, isFirstRow: true, rowSpan: 1 });
                return;
            }
    ...
    const downloadGlobalPDF = async () => {
        ...
        try {
            ...
            // ΓöÇΓöÇ Summary total row ΓöÇΓöÇ
    ...
    /* ΓöÇΓöÇ Download Seating Excel: one sheet per allocated hall ΓöÇΓöÇ */
        ...
        try {
            ...
            for (const hall of allocatedHalls) {
                ...
                const wsData: any[][] = [
                    ...
                    [`Seating Arrangement ΓÇö ${hall.hallCode}`],
    ...
    return (
        <div className="pb-12 bg-[#05080f] min-h-[calc(100vh-3.5rem)] font-sans text-slate-300 antialiased selection:bg-indigo-500/30 selection:text-indigo-200" style={{ backgroundImage: 'radial-gradient(circle at top right, rgba(30,58,138,0.1), transparent 40%), radial-gradient(circle at bottom left, rgba(49,46,129,0.15), transparent 40%)' }}>
            ...
            <Modal isOpen={!!detailHall} onOpenChange={(open: boolean) => { if (!open) { setDetailHall(null); loadSummary(); } }} backdrop="blur" size="full" scrollBehavior="inside"
                ...
                <ModalContent>
                    {() => (<>
                        ...
                        {/* ΓöÇΓöÇ Body with subtle grid pattern ΓöÇΓöÇ */}
                        <ModalBody className="p-8" style={{
                            ...
                            {detailLoading ? (
                                ...
                                <div>
                                    ...
                                    {/* ΓöÇΓöÇ Bench Grid ΓÇö Desk-style ΓöÇΓöÇ */}
            ...
            {/* ΓòÉΓòÉΓòÉ Print Modal (Professional Official Document Layout) ΓòÉΓòÉΓòÉ */}
                            ...
                            <ModalBody className="p-0 overflow-visible">
                                ...
                                <div id="seat-sync-print-area" style={{ fontFamily: "'Georgia', 'Times New Roman', serif", background: '#fff', color: '#111' }}>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', background: '#f8fafc', borderBottom: '1px solid #e2e8f0', padding: '14px 40px' }}>
                                        {[
                                            { label: 'EXAMINATION HALL', value: detailHall?.hallCode ?? 'ΓÇö' },
                                            { label: 'DATE', value: selectedDate ? fmtDate(selectedDate) : 'ΓÇö' },
                                            { label: 'SESSION', value: selectedSession === 'FN' ? 'Forenoon  (09:00 ΓÇô 12:00)' : 'Afternoon  (14:00 ΓÇô 17:00)' },
                                        ...
                                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, fontFamily: "'Arial', sans-serif" }}>
                                            <thead>
                                                <tr style={{ background: '#0f172a' }}>
                                                    {[
                                                        { label: 'BENCH', w: '7%' },
                                                        { label: 'LEFT ΓÇö REG. NO.', w: '14%' },
                                                        { label: 'LEFT ΓÇö STUDENT NAME', w: '26%' },
                                                        { label: 'DEPT', w: '7%' },
                                                        { label: 'RIGHT ΓÇö REG. NO.', w: '14%' },
                                                        { label: 'RIGHT ΓÇö STUDENT NAME', w: '26%' },
                                                        { label: 'DEPT', w: '6%' },
                                                    ].map(col => (
                                                ...
                                                {detailBenches.map((b, idx) => {
                                                    ...
                                                    return (
                                                        <tr key={`${b.rowLabel}${b.benchNumber}`} style={{ background: rowBg }}>
                                                            ...
                                                            <td style={{ ...cellStyle, fontFamily: "'Courier New', monospace", fontWeight: 600, color: la ? '#0f172a' : '#cbd5e1', fontSize: 11 }}>{la?.registerNumber ?? 'ΓÇö'}</td>
                                                            <td style={{ ...cellStyle, color: la ? '#1e293b' : '#cbd5e1' }}>{la?.studentName ?? 'ΓÇö'}</td>
                                                            <td style={{ ...cellStyle, textAlign: 'center' }}>
                                                                {la ? <span style={{ background: '#eff6ff', color: '#2563eb', border: '1px solid #bfdbfe', borderRadius: 4, padding: '2px 6px', fontSize: 10, fontWeight: 700 }}>{la.deptCode}</span> : <span style={{ color: '#cbd5e1' }}>ΓÇö</span>}
                                                            </td>
                                                            <td style={{ ...cellStyle, fontFamily: "'Courier New', monospace", fontWeight: 600, color: ra ? '#0f172a' : '#cbd5e1', fontSize: 11 }}>{ra?.registerNumber ?? 'ΓÇö'}</td>
                                                            <td style={{ ...cellStyle, color: ra ? '#1e293b' : '#cbd5e1' }}>{ra?.studentName ?? 'ΓÇö'}</td>
                                                            <td style={{ ...cellStyle, textAlign: 'center' }}>
                                                                {ra ? <span style={{ background: '#f0fdf4', color: '#16a34a', border: '1px solid #bbf7d0', borderRadius: 4, padding: '2px 6px', fontSize: 10, fontWeight: 700 }}>{ra.deptCode}</span> : <span style={{ color: '#cbd5e1' }}>ΓÇö</span>}
                                                            </td>
                                                        </tr>
                                                    );
                                                })}
                                    ...
                                    <div style={{ padding: '0 40px 32px', borderTop: '1.5px solid #e2e8f0', marginTop: 4 }}>
                                        ...
                                        <div style={{ textAlign: 'center', marginTop: 24, fontSize: 9, color: '#94a3b8', fontFamily: "'Arial', sans-serif", letterSpacing: '0.06em' }}>
                                            Generated by Seat-Sync ┬╖ {new Date().toLocaleString('en-IN')} ┬╖ CONFIDENTIAL ΓÇö FOR OFFICIAL USE ONLY
                                        </div>
                                    </div>
