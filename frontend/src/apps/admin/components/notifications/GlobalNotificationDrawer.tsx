...
import { X, Bell, AlertTriangle, Calendar, MessageSquare } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { getMyNotifications, Notification, initNotificationSocket, markAllAsRead } from '../../services/notificationService';
                                    ...
                                    {notifications.map((notif) => (
                                        <div key={notif.id} className={`p-5 transition-colors cursor-pointer group relative ${!notif.isRead ? 'bg-indigo-50/30' : 'hover:bg-slate-50'}`}>
                                            <div className="flex gap-4">
                                                <div className={`w-10 h-10 rounded-full shrink-0 flex items-center justify-center ${notif.type === 'EMERGENCY' ? 'bg-red-100 text-red-600 animate-pulse' :
